...
{
   // Extract trade data (simplified - use JSON library in production)
   ...
   // Parse JSON and execute each position
...
)
{
   ...
   request.price = (orderType == "market") ? SymbolInfoDouble(symbol, SYMBOL_ASK) : entryPrice;
   request.sl = stopLoss;
   request.tp = takeProfit;
   request.deviation = (ulong)MAX_SLIPPAGE;
   request.magic = MAGIC_NUMBER;
   request.comment = "TradingBot-" + positionId;
   
   // Send order
   if(!OrderSend(request, result))
   {
      Print("OrderSend error: ", GetLastError());
      ReportExecution(tradeId, positionId, false, "", 0, "Order failed: " + IntegerToString(GetLastError()));
      return false;
   }
   
   if(result.retcode == TRADE_RETCODE_DONE)
   {
      Print("Position opened: ", result.order, " for ", symbol);
      ReportExecution(tradeId, positionId, true, IntegerToString(result.order), result.price, "");
      return true;
   }
   else
   {
      Print("Order failed: ", result.retcode, " - ", result.comment);
      ReportExecution(tradeId, positionId, false, "", 0, result.comment);
      return false;
   }
}
...
void SendPing()
{
   ...
   // Build JSON payload
   string payload = "{";
   payload += "\"accountNumber\":\"" + IntegerToString(AccountInfoInteger(ACCOUNT_LOGIN)) + "\",";
